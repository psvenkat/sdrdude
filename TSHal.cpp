...
_Bool TS_HasNewTouchEvent(void)
{
    ...
    if (oldstate){
	debug(TOUCH, "New Touch Event at X=%d Y=%d Z=%d\n", tp.x, tp.y, tp.z);
    }
    return oldstate;
	//TSDriver_HasNewTouchEvent();
}

void  TS_GetTouchEventCoords(uint16_t *pX, uint16_t *pY)
{
	// Apply calibration:
	*pX = getDisplayCoordinateX(tp.x, tp.y);
	*pY = getDisplayCoordinateY(tp.x, tp.y);
}
...
//	int16_t sentinel = Read_Int_EEProm(EEPROM_SENTINEL_LOC);
	return 0;	//(sentinel == EEPROM_SENTINEL_VAL);
}
