...
void ScreenFrequencies_Create(void)
{
	...
	debug(GUI, "Populate Frequency Bands %d\n", FREQBAND_NUMBER_OF_BANDS);

	// Populate the frequency buttons:
	for (int i = 0; i < FREQBAND_NUMBER_OF_BANDS; i++) {
		GL_PageControls_TypeDef* btnFreq = NewButton(ID_FREQBTN_START + i, 
FrequencyManager_DisplayBandName(static_cast<BandPreset>(i)), freqButton_Click);
	...
	debug(GUI, "Populate Frequency Bands Done\n");
...
void Screen_ChangeButtonFreqLabel(int i){
	...
	ChangeButtonText(s_pThisScreen, ID_FREQBTN_START + i, FrequencyManager_DisplayBandName(static_cast<BandPreset>(i)));
...
static void freqButton_Click(GL_PageControls_TypeDef* pThis)
{
	...
	if (editMode==TRUE){
		FrequencyManager_SaveCurrentFrequency();
		FrequencyManager_SetSelectedBand(static_cast<BandPreset>(userBand));
...
//	displayNormalFeedback();
	...
	if (userMode<USERMODE_NUM_MODES-2)
		//userMode++;
		userMode = static_cast<UserModeType>( static_cast<int>(userMode)+1);
	else
