...
void GL_SetTextColor(__IO uint16_t GL_NewTextColor)
{
	...
	tft.setTextColor(GL_TextColor, GL_BackColor); 
}
...
{
	tft.setAddrWindow(Xpos, Ypos, Xpos+Width-1, Ypos+Height-1);
}
...
void GL_DrawLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length, uint8_t Direction)
{
  ...
  if (Direction == GL_Vertical) 
    tft.drawLine(Xpos, Ypos, Xpos, Ypos+Length, GL_TextColor);
  else
    tft.drawLine(Xpos, Ypos, Xpos+Length, Ypos, GL_TextColor);
}
...
void GL_LCD_DrawRect(uint8_t Xpos, uint16_t Ypos, uint8_t Height, uint16_t Width)
{
  ...
  tft.drawRect(Xpos, Ypos, Width, Height, GL_TextColor);
}
...
void GL_LCD_DrawFilledRect(uint16_t Xpos, uint16_t Ypos, uint16_t Height, uint16_t Width)
{
  ...
  tft.fillRect((int16_t)Xpos, (int16_t)Ypos, 
    (int16_t)Width, (int16_t)Height, GL_TextColor);
}
...
**/
  tft.drawCircle(Xpos, Ypos, Radius, GL_TextColor); 
}
...
{
  tft.drawRGBBitmap(x, y, pBitmap, width, height);
}
