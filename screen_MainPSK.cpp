...
static _Bool PGAUpdateHandler(GL_PageControls_TypeDef* pThis, _Bool forceRedisplay){
	int16_t x = Options_GetValue(static_cast<int>(OPTION_RX_RF));
	char buf[6] = "     ";
	String str = " " + String(x);
...
static _Bool DACUpdateHandler(GL_PageControls_TypeDef* pThis, _Bool forceRedisplay){
	int x = Options_GetValue(static_cast<int>(OPTION_RX_AUDIO));
	char buf[6] = "     ";
	//GL_SetTextColor(LCD_COLOR_BLACK);
...
static _Bool RSLUpdateHandler(GL_PageControls_TypeDef* pThis, _Bool forceRedisplay){
	int x = Options_GetValue(static_cast<int>(OPTION_RSL_CAL));
	char buf[6] = "     ";
	String str = String(x);
	str.toCharArray(buf, 6);
	GL_PrintString(277, 80, buf, 0);
	return 0;
}
...
void ScreenMainPSK_Create(void)
{
	...
	// Keyboard status
	pKeyboardLabel = Widget_NewLabel("Your keyboard...", LCD_COLOR_YELLOW, LCD_COLOR_BLACK, 0,
		GL_FONTOPTION_8x8,KeyboardStatusUpdateHandler);
	AddPageControlObj(LCD_WIDTH/2 -150, LCD_HEIGHT - 40, pKeyboardLabel, s_pThisScreen);
