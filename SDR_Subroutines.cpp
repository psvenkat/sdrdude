...
extern q15_t post_FILT_Coeff[125][5];
q15_t PFC[125];
...
void ChangeMode(int userMode){
	...
	Mode_SetCurrentMode(static_cast <UserModeType>(userMode));
...
void SetFrequency(char *string){
	...
	FrequencyManager_SetSelectedBand((BandPreset)15); //Blank the frequency band display
	FrequencyManager_SetCurrentFrequency((uint32_t)value);
}
...
void Sel_Filt1 ( void )
{
	...
	Fhigh = 3300;
	sprintf(sFilt,"F1");
	for (int i=0; i < POST_FILT_SIZE; i++)
...
void process_button ( void ){
	...
	unsigned char currOption;
	ButtonPress = Encoders_IsOptionsEncoderPressed();
	button_one_shot |= ButtonPress;
	if (button_one_shot) Press_cnt++;
	if (ButtonPress){
		currOption = static_cast<unsigned char>(Options_GetSelectedOption());
		Options_SetSelectedOption(static_cast<OptionNumber>(currOption+1));
	}
