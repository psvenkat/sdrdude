...
TSPoint_kbv TouchScreen_kbv::getPoint(void) {
    ...
    pinMode(_xp, OUTPUT);      //PSV
    digitalWrite(_xp, HIGH);   //PSV
    pinMode(_ym, OUTPUT);      //PSV
    digitalWrite(_ym, HIGH);   //PSV

    pinMode(_xm, OUTPUT);      //.kbv
    digitalWrite(_xm, HIGH);   //.kbv
    pinMode(_yp, OUTPUT);      //XM, YP returned to OUTPUT
    digitalWrite(_yp, HIGH);   //

    return TSPoint_kbv(x, y, z);  
}
