...
//**/  
	pinMode(I2CVDD, OUTPUT);
	digitalWrite(I2CVDD, HIGH);  //supply the 3.3V for the I2C
}
...
//**/ 
	  SWire.begin();
	  SWire.setRxBuffer(&rxBuffer, 32);
	  SWire.setTxBuffer(&txBuffer, 32);
}
...
{
	uint32_t result = 1;
	SWire.beginTransmission(DeviceAddr);
	SWire.write(RegisterAddr); // WRITE THE BYTE ADRESS (REGISTER)
	SWire.write(RegisterValue); // write data
	result = SWire.endTransmission();

	debug (CONTROL, "I2C_WriteRegister = %x\n", result);
		...
		uint8_t write[], uint8_t N)
{
	...
	for (uint8_t i=0; i<N; i++){
		SWire.write(write[i]); // write data
	}
	result = SWire.endTransmission();

	debug (CONTROL, "I2C_WriteRegister_N = %x\n", result);
...
uint32_t I2C_ReadSlave(uint8_t Address, uint8_t Register)
{
	...
	SWire.write(Register); // WRITE THE BYTE ADRESS (REGISTER)
	SWire.endTransmission(); 
	SWire.requestFrom(Address,1);
	result = SWire.read();     
