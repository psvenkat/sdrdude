...
uint8_t HS_DIV, N1;
uint64_t RFREQ, Old_RFREQ;
uint64_t FXTAL;
uint64_t F0 = 10000000L;
...
void Output_Frequency(long Freq_Out)
{
	...
	RFREQ = ((uint64_t) Freq_Out) * HS_DIV * N1 * 0X20000000UL / FXTAL;
	RFREQ /= 2;

	debug (CONTROL, "Output_Frequency %d \n", Freq_Out);
...
void Compute_FXTAL(void)
	...
	FXTAL = ( (uint64_t) F0 * HS_DIV * N1 * 0x40000000UL) / RFREQ;
	FXTAL = FXTAL / 4;  // get rid of extra x4 factor

	uint8_t RFREQ0 = RFREQ >> 32;
	debug (CONTROL, "Compute_FXTAL=%ld\n", FXTAL);
	debug (CONTROL, "\t F0=%ld\n", F0);
	debug (CONTROL, "\t HS_DIV=%ld\n", HS_DIV);
	debug (CONTROL, "\t N1=%ld\n", N1);
	debug (CONTROL, "\t RFREQ=%10LX", RFREQ);
	debug (CONTROL, "\t RFREQ=%X\n", RFREQ0);
	}
...
void Pack_Si570_registers(unsigned char reg[])
	...
	reg[3] = (RFREQ >> 16) & 0xFF;
	reg[2] = (RFREQ >> 24) & 0xFF;
	reg[1] = reg[1] | ((RFREQ >> 32) & 0b00111111);

	debug (CONTROL, "Pack_Si570_registers \n");
	debug (CONTROL, "  HS_DIV=%d    N1=%d \n", HS_DIV, N1);
	for (int i = 0; i < 6; i++){
		debug (CONTROL, " %2X\n", reg[i]);
		}
	}
...
void Unpack_Si570_registers(unsigned char reg[])
	...
	RFREQ = ((uint64_t) reg[2] << 24) + ( (uint32_t)reg[3] << 16) 
		+ ( (uint16_t) reg[4] << 8) + reg[5];
	RFREQ += (((uint64_t) reg[1] & 0b00111111) << 32);
	}
...
void Read_Si570_registers(unsigned char read[])
	{
	unsigned char i;
	debug (CONTROL, "Read_Si570_registers \n");
	for (i = 0; i < 6; i++){
