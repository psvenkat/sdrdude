...
static void displayFrequency(uint32_t number, uint16_t x, uint16_t y, uint32_t changeRate);
...
static void insideDrawHandler(GL_PageControls_TypeDef* pThis, _Bool force, int relX, int relY)
{
	//debug(GUI, "Widget_NewBigButtonFrequency:insideDrawHandler: X=%d  Y=%d\n", relX, relY);
	// Setup impossible values (as much as possible
	static BandPreset previousBand = static_cast<BandPreset>(-1);
...
static void displayFrequency(uint32_t number, uint16_t x, uint16_t y, uint32_t changeRate)
{
	//debug(GUI, "Widget_BigButtonFrequency:displayFrequency: X=%d  Y=%d\n", x, y);
	char buffer[BUFF_SIZE] = "         0";
	writeNumberToBuffer(number, buffer);

	// Display the whole number (in white):
