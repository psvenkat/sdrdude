...
int		WF_Line0 ;
int		WF_Count = 0;
/** PSV Keep Palette ix in the buffer, get the value later
	...
	else
	{
		if (WF_Count == 0){
			...
			for (int x = 0; x < FFT_WIDTH; x++)
			{
				...
				*(pWFBfr + (FFT_WIDTH*WF_Line0) + x) = FFT_Display[x + 8];
			}
			...
			for (int y = WF_Line0; y < FFT_HEIGHT; y++){
				for (int x = 0; x <FFT_WIDTH; x++) {
					...
					if ((x == NCO_Point && Mode_GetCurrentMode() == MODE_PSK)
						|| (x == 27 && Mode_GetCurrentMode() == MODE_CW)){
						//LCD_WriteRAM(LCD_COLOR_RED);
						GL_DrawPixel(x+xPos, y+yPos, LCD_COLOR_RED);
						//LCD_WriteRAM(LCD_COLOR_WHITE);
					}
					else {
						//LCD_WriteRAM(*(pWFBfr + y*FFT_WIDTH + x));
						//GL_DrawPixel(x+xPos, y+yPos, *(pWFBfr + y*FFT_WIDTH + x));
						GL_DrawPixel(x+xPos, y+yPos, WFPalette[*(pWFBfr + y*FFT_WIDTH + x)]);
					}
				}
			}
...
static void displaySMeter(int RSL)
{
	int x = 80;
	int y = 68;
	GL_SetDisplayWindow(x, y, SMETER_HEIGHT, FFT_WIDTH);
//PSV	LCD_WriteRAM_PrepareDir(LCD_WriteRAMDir_Down);
	...
	for (int x = 0; x < FFT_WIDTH; x++) {
		for (int y = 0; y < SMETER_HEIGHT; y++) {
			if (x <= Signal_Level) {
				...
				GL_DrawPixel(x+80, y+68, LCD_COLOR_GREEN);
			}
			else {
				//LCD_WriteRAM(LCD_COLOR_DGRAY);
				GL_DrawPixel(x+80, y+68, LCD_COLOR_DGRAY);
			}
		}
	}
...
static void displayFilterBW( void )
{
	int x = 80;
	int y = 66;
	GL_SetDisplayWindow(x, y, 2, FFT_WIDTH);
	...
	for (int x = 0; x < FFT_WIDTH; x++) {
		for (int y = 0; y < 2; y++) {
			if ((x >= x1)&&(x<=x2)) {
				...
				GL_DrawPixel(x+80, y+66, LCD_COLOR_YELLOW);
			}
			else {
				//LCD_WriteRAM(LCD_COLOR_BLACK);
				GL_DrawPixel(x+80, y+66, LCD_COLOR_BLACK);
			}
		}
	}
}
